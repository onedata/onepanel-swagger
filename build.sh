...
# Build Bash client
#
docker run --rm -e "CHOWNUID=${UID}" -v `pwd`:/swagger -t docker.onedata.org/bash-swagger-codegen:0.3.4 generate -i ./swagger.json -l bash -o ./generated/bash -c bash-config.json
