...
# in 'generated/python'
#
docker run --rm -e "CHOWNUID=${UID}" -v `pwd`:/swagger -t docker.onedata.org/swagger-codegen:2.2.2-1b1767e  generate -i ./swagger.json -l python -o ./generated/python -c python-config.json

#
# Build Bash client
#
#docker run --rm -e "CHOWNUID=${UID}" -v `pwd`:/swagger -t docker.onedata.org/bash-swagger-codegen:0.3.8 generate -i ./swagger.json -l bash -o ./generated/bash -c bash-config.json
